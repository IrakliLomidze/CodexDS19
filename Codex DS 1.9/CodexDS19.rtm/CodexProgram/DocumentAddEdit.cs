...
namespace ILG.Codex.CodexR4
{
    public partial class DocumentAddEdit : Form
    {
        ...
        public void NewDocumentMode()
        {
            ...
            ZipFileName = DirectoryConfiguration.DSTemporaryDirectory + "znew_" + DateTime.Now.Ticks.ToString() + ".zip";
        ...
        public int EditDocumentMode(int ID)
        {
            ...
            string CodexDocTempFilename = DirectoryConfiguration.DSTemporaryDirectory + "\\Codex_" + ID.ToString() + U1;

            while (System.IO.File.Exists(CodexDocTempFilename) == true)
            {
                U1 = DateTime.Now.Ticks.ToString();
                CodexDocTempFilename = DirectoryConfiguration.DSTemporaryDirectory + "\\Codex_" + ID.ToString() + U1;
            }

            U1 = DateTime.Now.Ticks.ToString();
            string CodexLinkTempFilename = DirectoryConfiguration.DSTemporaryDirectory + "\\CodexLink_" + ID.ToString() + U1;

            while (System.IO.File.Exists(CodexLinkTempFilename) == true)
            {
                U1 = DateTime.Now.Ticks.ToString();
                CodexLinkTempFilename = DirectoryConfiguration.DSTemporaryDirectory + "\\CodexLink_" + ID.ToString() + U1;
            }
            ...
            if ((HasAttachments == true) && (License.IsAttachmentShow() == true))
            {
                ...
                if (filesinzip.Count != 0)
                {
                    foreach (var f in filesinzip)
                    {
                        ...
                        string st1 = DirectoryConfiguration.DSTemporaryDirectory + "\\" + Path.GetFileName(f.ToString()).ToUpper().Trim();
                        st1 = st1.Replace("?", "_");
        ...
        private void OpenPDFDocument()
        {
            ...
            if (fd.ShowDialog() == DialogResult.OK)
            {
                ...
                string dstr = DirectoryConfiguration.DSTemporaryDirectory.ToString() + "\\" + Path.GetFileName(fd.FileName);
                while (File.Exists(dstr) == true)
                {
                    dstr = DirectoryConfiguration.DSTemporaryDirectory.ToString() + "\\" + DateTime.Now.Millisecond.ToString() + Path.GetFileName(fd.FileName);
                }
 ...
 //           CCategory.DisplayLayout.Grid.Width = CAuthor.Width;
      ...
      //  private C1.C1Zip.C1ZipFile zfD;
        ...
        private void CreareRTFDocument()
        {
            ...
            TxDocument.SaveToRTF(DirectoryConfiguration.DSTemporaryDirectory + @"\D.RTF");
            ...
            String ZipFileName = DirectoryConfiguration.DSTemporaryDirectory + @"\Doc2.Zip";
            String RTFFileName = DirectoryConfiguration.DSTemporaryDirectory + @"\D.RTF";
            ...
            FileStream fs = new FileStream(DirectoryConfiguration.DSTemporaryDirectory + @"\Doc2.Zip", FileMode.Open, FileAccess.Read);
            resultdoc = new byte[(int)fs.Length];
            fs.Read(resultdoc, 0, (int)fs.Length);
            fs.Close();
            File.Delete(DirectoryConfiguration.DSTemporaryDirectory + @"\Doc2.Zip");
            if (File.Exists(DirectoryConfiguration.DSTemporaryDirectory + @"\D.RTF") == true) File.Delete(DirectoryConfiguration.DSTemporaryDirectory + @"\D.RTF");
        ...
        private void CrearePDFDocument()
        {
            ...
            String ZipFileName = DirectoryConfiguration.DSTemporaryDirectory + @"\Doc3.Zip";
            ...
            FileStream fs = new FileStream(DirectoryConfiguration.DSTemporaryDirectory + @"\Doc3.Zip", FileMode.Open, FileAccess.Read);
            resultdoc = new byte[(int)fs.Length];
            fs.Read(resultdoc, 0, (int)fs.Length);
            fs.Close();
            File.Delete(DirectoryConfiguration.DSTemporaryDirectory + @"\Doc3.Zip");
            if (File.Exists(DirectoryConfiguration.DSTemporaryDirectory + @"\D.PDF") == true) File.Delete(DirectoryConfiguration.DSTemporaryDirectory + @"\D.RTF");
        ...
        private void CreareLinks()
        {
            ...
            String ZipFileName = DirectoryConfiguration.DSTemporaryDirectory + @"\LINKS.ZIP";
            String FileName_Links = DirectoryConfiguration.DSTemporaryDirectory + @"\Links.XML";
            String FileName_LinksSchema = DirectoryConfiguration.DSTemporaryDirectory + @"\LinksSchema.XML";
            ...
            File.Delete(DirectoryConfiguration.DSTemporaryDirectory + @"\Links.XML");
            File.Delete(DirectoryConfiguration.DSTemporaryDirectory + @"\LinksSchema.XML");

            FileStream fs = new FileStream(DirectoryConfiguration.DSTemporaryDirectory + @"\LINKS.ZIP", FileMode.Open, FileAccess.Read);
            linkresult = new byte[fs.Length];
            fs.Read(linkresult, 0, (int)fs.Length);
            fs.Close();
            File.Delete(DirectoryConfiguration.DSTemporaryDirectory + @"\LINKS.ZIP");
        }
